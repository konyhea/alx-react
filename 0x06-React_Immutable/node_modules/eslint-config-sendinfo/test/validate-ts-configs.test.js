...
describe('Validate TS configs', () => {
  ...
  it('Validate eslint-config-ali/essential/typescript/react', () => {
    ...
    // 验证黑名单中的 react-hooks 规则已关闭，取 react-hooks/rules-of-hooks 进行测试
    const filePath2 = path.join(__dirname, './fixtures/react-hooks.jsx');
    const reports2 = cli.executeOnFiles([filePath2]);
    assert.ok(reports2.errorCount !== 0 || reports2.warnCount !== 0);
    const results2 = reports2.results[0];
    const hooksIssue = results2.messages.find(result => {
      return result.ruleId === 'react-hooks/rules-of-hooks';
    });
    assert.equal(hooksIssue.severity, 1);
  });
